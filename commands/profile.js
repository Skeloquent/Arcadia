...
module.exports.run = async (client, message, args) => {
  ...
  console.log(`# Command used: ${client.clientconfig.prefix}profile || In ${message.guild.name}`);
...
//console.log('game: '+game+' player: '+player)
  ...
  if (client.Users[game][player].image) {
  rich.setImage(client.Users[game][player].image)
}


if(client.Users[game][player].flavor!="") {
  rich.setFooter(`${client.Users[game][player].flavor}`+'\nLast updated on '+client.Users[game][player].time)
}

else {

  rich.setFooter('Last updated on '+client.Users[game][player].time)
}
...
module.exports.help = {
  name: "profile",
  usage: `profile/profile2 <game_name> <User_ping/Discord ID>`,
